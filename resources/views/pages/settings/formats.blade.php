    ...
    <section class="w-screen h-screen pl-[80px] py-4 text-gray-700">
        ...
        <div class="height-kategorije pb-[30px] scroll">
            <div class="flex items-center px-[50px] py-8 space-x-3 rounded-lg">
                <a href="{{route('settings.formats.create')}}"
                            ...
                            <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                                ...
                                <div
                                    ...
                                    <div
                                        ...
                                        role="menu">
                                        <div class="py-1">
                                            <a href="{{ route('settings.formats.edit', $format->id) }}" tabindex="0"
                                               class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                               role="menuitem">
                                                <i class="fas fa-edit mr-[1px] ml-[5px] py-1"></i>
                                                <span class="px-4 py-0">Izmijeni</span>
                                            </a>
                                            <form onSubmit="if(!confirm('Da li ste sigurni da želite da obrišete ovaj format?')){return false;}" method="POST" action="{{route('settings.formats.destroy', $format->id)}}">
                                                @csrf
                                                @method('DELETE')
