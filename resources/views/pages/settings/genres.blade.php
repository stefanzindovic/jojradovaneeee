...
@section('page_content')
    ...
    <div class="height-kategorije pb-[30px] scroll">
        <div class="flex items-center px-[50px] py-8 space-x-3 rounded-lg">
            <a href="{{ route('settings.genres.create') }}"
            ...
            <table class="overflow-hidden shadow-lg rounded-xl min-w-full border-[1px] border-[#e4dfdf]" id="myTable">
                <thead class="bg-[#EFF3F6]">
                    <tr class="border-b-[1px] border-[#e4dfdf]">
                        <th data-orderable="false" class="px-4 py-4 leading-4 tracking-wider text-left text-blue-500">
                            <label class="inline-flex items-center">
                                <input type="checkbox" class="form-checkbox">
                            </label>
                        </th>
                        <th class="px-4 py-4 leading-4 tracking-wider text-left">Naziv žanra
                        </th>
                    ...
                    <td class="px-4 py-4 text-sm leading-5 text-right whitespace-no-wrap">
                        ...
                        <div
                            ...
                            <div
                                ...
                                <div class="py-1">
                                    <a href="{{route('settings.genres.edit', $genre->id)}}" tabindex="0"
                                       class="flex w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 outline-none hover:text-blue-600"
                                       role="menuitem">
                                        <i class="fas fa-edit mr-[1px] ml-[5px] py-1"></i>
                                        <span class="px-4 py-0">Izmijeni</span>
                                    </a>
                                    <form onSubmit="if(!confirm('Da li ste sigurni da želite da obrišete ovaj žanr?')){return false;}" method="POST" action="{{route('settings.genres.destroy', $genre->id)}}">
                                        @csrf
                                        @method('DELETE')
