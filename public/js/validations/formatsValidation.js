        ...
        if (
            ...
            setTimeout(function () {
                nameValidationMessage
                    .css({ display: "block" })
                    .html(
                        '<small class="text-red-500 align-middle"><i class="fa fa-times text-red"></i> Vrijednost polja može sadržati od 2 do 25 karaktera.</small>'
                    );
            }, 200);
            e.preventDefault();
        } else if (!/^([a-zA-Z0-9-._\sŠšĐđŽžČčĆć])+$/.test(nameInput.val())) {
            setTimeout(function () {
                nameValidationMessage
                    .css({ display: "block" })
                    .html(
